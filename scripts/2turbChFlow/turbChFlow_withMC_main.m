function [ ] = turbChFlow_withMC_main(data, caseName, plotp, plotU,plotUrms,plotuv)
%% Make new dirs
...
addpath([SCRIPTS '/DNS']);  % mounted data

Udns = dlmread("UMean.txt", '', 2, 0);
Rdns = dlmread("RMean.txt", '', 2, 0);

load('/home/jigar/Dropbox/1Groningen/PhD/14PaperThesis/1_UQ_IPC_SIAM/figures/2turbChFlow/MC_data.mat')
...
legBool    = 'off';
leg_MC = {'$\mu_{\mathrm{IPC}}$','$\mu_{\mathrm{MC}}$',...
...
figure(4)
    ...
    axis([0 1 0 30])
    leg1 = legend([plt2 plt3 fl1 fl2], leg_MC,'Location','southeast');
    set(leg1,'Visible','on');
    hold off;
...
figure(5)
    ...
    axis([0 1 0 ymax])
%     legend([plt2 fl1],{'DNS','$\mu_{u_{rms}}$','$\pm 2\sigma_{u_{rms}}$'});
    leg1 = legend([plt2 plt3 fl1 fl2], leg_MC,'Location','southeast');
    set(leg1,'Visible',legBool);
    hold off;
...
figure(6)
    ...
    axis([0 1 0 1])
%     legend([plt2 fl1],{'DNS','$\mu_u$','$\pm 2\sigma_u$'});
    leg1 = legend([plt2 plt3 fl1 fl2], leg_MC,'Location','southeast');
    set(leg1,'Visible',legBool);
    hold off;
...
figure(66)
    ...
    set(gca,'XScale','log')
    axis([0 400 0 1])
%     legend([ plt2 fl1],{'DNS','$\mu_u$','$\pm 2\sigma_u$'},'Location','Best');
    leg1 = legend([plt2 plt3 fl1 fl2], leg_MC,'Location','southeast');
    set(leg1,'Visible',legBool);
    hold off;
