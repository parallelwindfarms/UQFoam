function [ ] = turbChFlow_withDNS_withMC_withDET_main_DFSEM(data, caseName, ...
...
%DET
load([data '/DET/DET_tut_M1/DET_data.mat']);
% load([data '/DET/DET_tut_M2/DET_data.mat']);
...
% addpath("./postProcessing/collapsedFields/25000");
files = [   
            "pMean0.xy"; "pMeanSigma.xy";
            "pMean1.xy"; "pMean2.xy";

            "nutMean0.xy"; "nutMeanSigma.xy";
            "nutMean1.xy"; "nutMean2.xy";

            "UMean0_X.xy"; "UMeanSigma_X.xy";
            "UMean1_X.xy"; "UMean2_X.xy";
            
            "RMean0_XX.xy"; "RMean0_YY.xy"; "RMean0_ZZ.xy"; "RMean0_XY.xy";
            "RMean1_XX.xy"; "RMean1_YY.xy"; "RMean1_ZZ.xy"; "RMean1_XY.xy";
            "RMean2_XX.xy"; "RMean2_YY.xy"; "RMean2_ZZ.xy"; "RMean2_XY.xy";
...
legBool        = 'off';
leg_DNS_MC_IPC = {'DNS','DET','$\mu_{\mathrm{IPC}}$','$\mu_{\mathrm{MC}}$',...
...
ut_nu 	= ut/nu;
yPlus   = y*ut_nu;
utDET   = ut;                   % yPlus_DET = 1.68

MC_ut     = 0.007;              % Theo's MC simulations
MC_factor = MC_ut/ut;           % 1 = org

dns  = Udns(:,3);
det  = UMean_X_DET(:,col2)/utDET;

Mean  = UMean0_X(:,col2)/ut;
% Sigma = UMeanSigma_X(:,col2)/ut;
Sigma = sqrt(UMean1_X(:,col2).^2+UMean2_X(:,col2).^2)/ut;
% Sigma = UMean1_X(:,col2)/ut;
% Sigma = UMean2_X(:,col2)/ut;
Ub    = Mean+N*Sigma;
Lb    = Mean-N*Sigma;
...
figure(4)
    ...
    axis([0 1 0 25])
    leg1 = legend([plt1 plt4 plt2 plt3 fl1 fl2], leg_DNS_MC_IPC,'Location','southeast');
    set(leg1,'Visible','on');
    set(leg1,'Color','none');
    set(leg1,'EdgeColor','none');
    hold off;
    
%     set(figure(4), 'Position',[2.5 1.5 8 6])
%     set(gcf, 'PaperPosition', [2.5 1.5 8 6]);
...
ut_nu 	= ut/nu;
yPlus   = y*ut_nu;
ut2DET  = (1.68*nu/yWall)^2;    % yPlus_DET = 1.68

MC_ut     = 0.007;              % Theo's MC simulations
MC_factor = 1;%MC_ut/ut;        % 1 = org

dns  = sqrt(Rdns(:,3:5));
det  = sqrt([(UPrime2Mean_XX_DET(:,col2)), (UPrime2Mean_YY_DET(:,col2)), ...
             (UPrime2Mean_ZZ_DET(:,col2))]/ut2DET);
...
% Sigma2 = [(RMean1_XX(:,col2)), (RMean1_YY(:,col2)), (RMean1_ZZ(:,col2))];
% Sigma2 = [(RMean2_XX(:,col2)), (RMean2_YY(:,col2)), (RMean2_ZZ(:,col2))];
Ub   = sqrt(Mean+N*Sigma2)/ut;
Lb   = sqrt(Mean-N*Sigma2)/ut;
Mean = sqrt(Mean)/ut;

MC_Mean = [RMean0_XX_MC', RMean0_YY_MC', RMean0_ZZ_MC'];
MC_Ub   = sqrt(MC_Mean+MC_N*[RMeanSigma_XX_MC', RMeanSigma_YY_MC', RMeanSigma_ZZ_MC'])*MC_factor;
MC_Lb   = sqrt(MC_Mean-MC_N*[RMeanSigma_XX_MC', RMeanSigma_YY_MC', RMeanSigma_ZZ_MC'])*MC_factor;
MC_Mean = sqrt(MC_Mean)*MC_factor;
...
%     ylabel("$( v_{rms} , w_{rms} + u_\tau , u_{rms} + 2u_\tau ) / u_\tau$")
    ...
    axis([0 1 0 ymax])
%     legend([plt1 plt4 plt2 fl1],{'DNS','$\mu_{u_{rms}}$','$\pm 2\sigma_{u_{rms}}$'});
    leg1 = legend([plt1 plt4 plt2 plt3 fl1 fl2], leg_DNS_MC_IPC,'Location','southeast');
    set(leg1,'Visible',legBool);
    set(leg1,'Color','none');
    set(leg1,'EdgeColor','none');
...
dns  = -Rdns(:,6);
det  = -UPrime2Mean_XY_DET(:,col2)/ut2DET;

Mean  = -RMean0_XY(:,col2)/ut2;
% Sigma = RMeanSigma_XY(:,col2)/ut2;
Sigma = sqrt(RMean1_XY(:,col2).^2+RMean2_XY(:,col2).^2)/ut2;
% Sigma = RMean1_XY(:,col2)/ut2;
% Sigma = RMean2_XY(:,col2)/ut2;
Ub = Mean+N*Sigma;
Lb = Mean-N*Sigma;

MC_Mean = -RMean0_XY_MC'*MC_factor;
MC_Ub   = MC_Mean+MC_N*RMeanSigma_XY_MC'*MC_factor;
MC_Lb   = MC_Mean-MC_N*RMeanSigma_XY_MC'*MC_factor;
...
figure(6)
    ...
    axis([0 1 0 1])
%     legend([plt1 plt4 plt2 fl1],{'DNS','$\mu_u$','$\pm 2\sigma_u$'});
    leg1 = legend([plt1 plt4 plt2 plt3 fl1 fl2], leg_DNS_MC_IPC,'Location','northeast');
...
figure(66)
    ...
    set(gca,'XScale','log')
    axis([0 400 0 1])
%     legend([plt1 plt4 plt2 fl1],{'DNS','$\mu_u$','$\pm 2\sigma_u$'},'Location','Best');
    leg1 = legend([plt1 plt4 plt2 plt3 fl1 fl2], leg_DNS_MC_IPC,'Location','northeast');
    set(leg1,'Visible',legBool);
    set(leg1,'Color','none');
    set(leg1,'EdgeColor','none');
