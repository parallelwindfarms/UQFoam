...
def getSymmTensorValues(symmTensor, nCells):
    tensor = np.zeros((nCells, 6))

    tensor[:,0],tensor[:,1],tensor[:,2],\
    tensor[:,3],tensor[:,4],tensor[:,5] = \
    symmTensor[:,0,0],symmTensor[:,0,1],symmTensor[:,0,2],\
    symmTensor[:,1,1],symmTensor[:,1,2],symmTensor[:,2,2]
...
# %% Add sampled perturbation to mean field of eigenvalues
